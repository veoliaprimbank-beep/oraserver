...
async function agentEditor(articles) {
  ...
  try {
    ...
    const text = await callAI(
      `Ești editorul portalului de știri ORA.MD din Moldova. Fă titlurile mai interesante și atractive pentru cititorii din Moldova. Păstrează esența dar adaugă intrigă. Răspunde DOAR cu un array JSON de șiruri fără explicații.\n\nTitluri:\n${list}\n\nArray JSON cu ${toProcess.length} șiruri:`
    );
...
// Агент 2: Переводчик RO → RU (отключён — статьи показываются на оригинальном языке)
...
async function agentSummarizer(articles) {
  ...
  try {
    ...
    const text = await callAI(
      `Pentru fiecare știre scrie un rezumat scurt (1 propoziție, max 90 caractere). Răspunde DOAR cu un array JSON de șiruri.\n\n${texts}\n\nArray JSON cu ${toSum.length} șiruri:`
    );
...
async function agentTrends(articles) {
  ...
  try {
    ...
    const text = await callAI(
      `Analizează știrile din Moldova și identifică 5 tendințe principale ale zilei. Pentru fiecare: nume (3-4 cuvinte), numărul de știri asociate, emoji potrivit. Răspunde DOAR cu un array JSON de obiecte {topic, count, emoji}.\n\nȘtiri:\n${titles}\n\nArray JSON cu 5 obiecte:`,
      400
    );
...
async function agentDigest(articles) {
  ...
  try {
    ...
    const text = await callAI(
      `Ești editorul ORA.MD. Creează un digest de dimineață în română. Format: titlu, introducere (2 propoziții), top-5 știri cu explicație scurtă. Stil profesional și viu. Răspunde DOAR cu obiect JSON {title, intro, items:[{headline,summary}]}.\n\nȘtiri:\n${top}\n\nJSON:`,
      800
    );
...
async function parseAll() {
  ...
  // AI агенты — параллельно для скорости
  if (process.env.ANTHROPIC_API_KEY) {
    console.log('\n─── AI агенты (параллельно) ───');
...
app.get('/api/agents',(req,res)=>res.json({
  ...
  agents:{
    editor:     {name:'Editor titluri',    ...cache.agentStats.editor},
    translator: {name:'Traducător RO→RU',  ...cache.agentStats.translator},
    summarizer: {name:'Rezumate',          ...cache.agentStats.summarizer},
    trends:     {name:'Analiză tendințe',  ...cache.agentStats.trends},
    digest:     {name:'Digest dimineață',  ...cache.agentStats.digest},
  }
}));
...
app.get('/api/status',(req,res)=>res.json({
  ...
  ai:{enabled:!!process.env.ANTHROPIC_API_KEY,...cache.agentStats},
...
app.listen(PORT, async () => {
  console.log(`
╔══════════════════════════════════════════╗
║    ORA.MD — RSS + AI Server v4.0        ║
║    http://localhost:${PORT}                ║
╠══════════════════════════════════════════╣
║  🤖 Editor titluri (RO)                  ║
║  📝 Rezumate (RO)                        ║
║  📈 Analiză tendințe                     ║
║  🗞️  Digest dimineață (9:00)             ║
║  📡 13 surse RSS                         ║
╠══════════════════════════════════════════╣
║  AI: ${process.env.ANTHROPIC_API_KEY ? '✅ ACTIV' : '⚠️  Lipsă ANTHROPIC_API_KEY'}
╚══════════════════════════════════════════╝
  `);
  await parseAll();
});
